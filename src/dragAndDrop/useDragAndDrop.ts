...
function useDragAndDrop() {
	...
	const handleDragStart = ({
		...
		const transparentClone = draggingElement.current.cloneNode(
			true,
		) as HTMLElement;
		...
		(startY as React.MutableRefObject<number>).current =
			rect.top + rect.height / 2;
		(tempY as React.MutableRefObject<number>).current = rect.top;
		(startX as React.MutableRefObject<number>).current =
			rect.left + rect.width / 2;
		(tempX as React.MutableRefObject<number>).current =
			rect.left + rect.width / 2;
	...
	const handleHorizontalDrag = ({
		...
		Object.entries(listItemPositions.current).forEach(([key, value]) => {
			if (Number.parseInt(key) === tempDragItem.current) return;
			...
			if (!notShifted) {
				shiftLeft
					? ((tempDragItem as React.MutableRefObject<number>).current =
	...
	const handleVerticalDrag = ({
		...
		const shiftY =
			event.clientY - (startY as React.MutableRefObject<number>).current;
		const currentY = event.clientY;
		const height = clone.offsetHeight / 2;

		if (container.top < currentY && currentY < container.bottom - height) {
		...
		if (isScrolling.current) return;
		Object.entries(listItemPositions.current).forEach(([key, value]) => {
			if (Number.parseInt(key) === tempDragItem.current && !value.shifted)
				return;

			if (
				((tempY as React.MutableRefObject<number>).current > value.y &&
					currentY > value.y) ||
				((tempY as React.MutableRefObject<number>).current < value.y &&
					currentY < value.y)
			)
			...
			const index = Number.parseInt(key);
			const notShifted =
				((dragItem as React.MutableRefObject<number>).current < index &&
	...
	const handleDragEnter = ({
		...
		cloneId,
		partialId,
	}: { index: number; cloneId: string; partialId: string }) => {
	...
	const handleAfterDrop = ({
		...
		data.forEach((_, index) => {
			...
			const cells = (table as HTMLElement).querySelectorAll(
